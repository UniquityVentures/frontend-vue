...
export const createViewset = (baseUrl, filename = 'export') => ({
    ...
    retrieve: async (id) => 
        (await api.get(`${baseUrl}/${id}`)).data,

    // Create new record
    create: async (data) => 
        (await api.post(`${baseUrl}/create/`, data)).data,

    // Update existing record
    update: async (data) =>
        (await api.put(`${baseUrl}/${data.id}/`, data)).data,
    ...
    import: {
        dryRun: async (file) => {
            ...
            return (await api.post(`${baseUrl}/import/dry_run/`, formData, {
        ...
        finalize: async (file) => {
            const formData = new FormData();
            formData.append('file', file);
            return (await api.post(`${baseUrl}/import/final/`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            })).data;
        }
    },
    ...
    stats: async () => 
        (await api.get(`${baseUrl}/stats/`)).data
});
